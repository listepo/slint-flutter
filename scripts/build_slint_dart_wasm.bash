...
# the `wasm-bindgen` entry points in `rust/wasm.rs` instead. Copy both output
# files into a Flutter application's `web/` directory and pass the JavaScript
# one to `initSlint()`.
#
...
# gets its frames from the embedded surface in `rust/embedded.rs`, which
# Flutter paints into the widget tree. Winit comes along regardless —
